...
segType  = zeros(nSeg,1);   % 1=fixed, 2=cruise, 3=loiter  (saved for reuse)
segC     = nan(nSeg,1);     % sfc, cruise/loiter segments only
segV     = nan(nSeg,1);     % velocity, cruise segments only
segLD    = nan(nSeg,1);     % L/D, cruise/loiter segments only
segR     = nan(nSeg,1);     % range used, cruise segments only (ft)
segE     = nan(nSeg,1);     % endurance used, loiter segments only (s)
isRangeLeg = false(nSeg,1); % true if this cruise leg should be swept in a range trade
...
for i = 1:nSeg
    ...
    switch stype
        ...
        case 2
            ...
            rl = input('  Mark this leg as a RANGE-TRADE candidate for later? (y/n): ', 's');
        ...
        case 3
            ...
            LD = input('  Lift-to-drag ratio L/D             : ');
            frac(i) = exp(-E*Clt/LD);
            segE(i) = E; segC(i) = Clt; segLD(i) = LD;
            fprintf('  --> Computed Wi/W(i-1) = %.4f\n', frac(i));
...
%% ---------------------------------------------------------
%  STEP 6b: SAVE MISSION DEFINITION FOR REUSE
%  (so range_trade.m, payload_trade.m, etc. can load this
%   instead of re-entering the whole mission profile)
% ---------------------------------------------------------
save('mission_data.mat', 'names','A_tab','C_tab','A','C','K_vs','acType', ...
     'nSeg','frac','segLabel','segType','segC','segV','segLD','segR','segE', ...
     'isRangeLeg','Wcrew','Wpayload','Wfixed', ...
     'Wx_W0','Wf_W0','trapped_reserve','W0_guess');
fprintf('Mission data saved to "mission_data.mat" - range_trade.m and similar scripts\n');
fprintf('can load this automatically instead of re-entering the whole mission.\n\n');
